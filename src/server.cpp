...
void ServiceManager::stop()
{
    ...
    for (auto& servicePortIt : acceptors) {
        try {
            ...
            post(io_service, [ObjectPtr = servicePortIt.second] { ObjectPtr->onStopServer(); });
#else
...
Protocol_ptr ServicePort::make_protocol(const bool checksummed, NetworkMessage& msg, const Connection_ptr& connection) const
{
...
void ServicePort::openAcceptor(const std::weak_ptr<ServicePort>& weak_service, const uint16_t port)
{
