...
GameStore.insertHistory = function(accountId, mode, description, amount, coinType)
    return db.asyncQuery(string.format(
                             'INSERT INTO `store_history`(`account_id`, `mode`, `description`, `coin_amount`, `time`, `coin_type`) VALUES (%u, %u, %s, %d, %u, %u)',
                             accountId, mode, db.escapeString(description), amount, os.time(), coinType))
end
...
function addOffer(player, offer, omsg)
    ...
    if (offer.type ~= GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE and offer.type ~=
        GameStore.OfferTypes.OFFER_TYPE_EXPBOOST and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_PREYSLOT and
        offer.type ~= GameStore.OfferTypes.OFFER_TYPE_PREYBONUS and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_TEMPLE and
        offer.type ~= GameStore.OfferTypes.OFFER_TYPE_SEXCHANGE and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_POUCH and
        not offer.id) then
        disabled = true
    end
    ...
    if (not disabled) then
        ...
        elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_ALLBLESSINGS) then
            ...
            -- end
        elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT or offer.type ==
            GameStore.OfferTypes.OFFER_TYPE_OUTFIT_ADDON) then
        ...
        elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_PROMOTION) then
            ...
            -- end
        elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_PREYSLOT or offer.type ==
            GameStore.OfferTypes.OFFER_TYPE_PREYBONUS) then
            disabled = true
            disabledReason = 'This category ain\'t done yet.'
            ...
            -- end
        elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_EXPBOOST or offer.type ==
            GameStore.OfferTypes.OFFER_TYPE_HOUSE) then
            disabled = true
            disabledReason = 'This category ain\'t done yet.'
...
function playerTransferCoins(resultId, playerId, amount)
    ...
    if (balance < amount) then
        GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_TRANSFER,
    ...
    if (accountId == player:getAccountId()) then
        GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_TRANSFER,
                                 'You can\'t transfer coins to a character in the same account.')
        return
    end
    ...
    GameStore.insertHistory(accountId, GameStore.HistoryTypes.HISTORY_TYPE_NONE,
                            player:getName() .. ' transfered you this amount.', amount,
                            GameStore.CoinTypes.TRANSFERABLE_TIBIA_COIN)
    GameStore.insertHistory(player:getAccountId(), GameStore.HistoryTypes.HISTORY_TYPE_NONE,
                            'You transfered this amount to ' .. receiver, -1 * amount,
                            GameStore.CoinTypes.TRANSFERABLE_TIBIA_COIN)
...
function parseTransferCoins(player, msg)
    ...
    if (amount < packages) then
        GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_TRANSFER,
                                 'You can\'t transfer this amount of coins.')
        return
    end
    ...
    db.asyncStoreQuery('SELECT `account_id`, `name`, (SELECT `coins` FROM `accounts` WHERE `id` = ' ..
                           player:getAccountId() .. ') as `coins` FROM `players` WHERE `name` = ' ..
                           db.escapeString(receiver), playerTransferCoins, player:getId(), amount)
end
...
function parseOpenStore(player, msg)
    ...
    omsg:sendToPlayer(player)
    omsg:delete()
    db.asyncStoreQuery('SELECT `coins` FROM `accounts` WHERE `id` = ' .. player:getAccountId(), playerUpdateCoins,
                       player:getId())
end
...
function playerBuyStore(resultId, playerId, offerId, productType, extraInfo)
    ...
    if (offer.coin == GameStore.CoinTypes.TOURNAMENT_COIN) then
        GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                 'We don\'t support tournament coins yet.')
        return
    end
    ...
    if (balance < offer.price) then
        GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                 'You don\'t have enough coins. Your purchase has been cancelled.')
        return
    end
    ...
    if (offer.disabled or offer.type == GameStore.OfferTypes.OFFER_TYPE_NONE) then
        if (offer.disabledReason) then
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                     'This offer is disabled: ' .. offer.disabledReason)
        else
    ...
    elseif (offer.type ~= GameStore.OfferTypes.OFFER_TYPE_EXPBOOST and offer.type ~=
        GameStore.OfferTypes.OFFER_TYPE_PREYSLOT and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_PREYBONUS and
        offer.type ~= GameStore.OfferTypes.OFFER_TYPE_TEMPLE and offer.type ~= GameStore.OfferTypes.OFFER_TYPE_SEXCHANGE and
        offer.type ~= GameStore.OfferTypes.OFFER_TYPE_POUCH and not offer.id) then
        GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE, 'This offer is unavailable.')
        return
    end
    ...
    if (offer.type == GameStore.OfferTypes.OFFER_TYPE_ITEM) then
        ...
        if (player:getFreeCapacity() < ItemType(itemId):getWeight(offer.count)) then
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                     'Please make sure you have free capacity to hold this item.')
            return
        end
        ...
        if (inbox) then -- check if we have store inbox
            ...
            if (not player:addItem(itemId, offer.count, false)) then
                GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                         'Please make sure you have free slots in your inventory.')
                return
            end
        end
    ...
    elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT or offer.type ==
        ...
        if (outfitLookType) then
            if (offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT and player:hasOutfit(outfitLookType)) then
                GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                         'This offer is disabled: You already have this outfit.')
                return
            elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_OUTFIT_ADDON) then
                if (player:hasOutfit(outfitLookType)) then
                    if (player:hasOutfit(outfitLookType, offer.addon)) then
                        GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                                 'This offer is disabled: You already have this addon.')
                        return
                    end
                else
                    GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                             'This offer is disabled: You don\'t have the outfit, you can\'t buy the addon.')
                    return
                end
            end

            if (not player:addOutfitAddon(outfitLookType, offer.addon) or
                not player:hasOutfit(outfitLookType, offer.addon)) then
                GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                         'There has been an issue with your outfit purchase. Your purchase has been cancelled.')
                return
            end
        else
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                     'This offer is disabled: The offer is fake.')
            return
        end
    elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_MOUNT) then
        local hasMount = player:hasMount(offer.id)
        if (hasMount) then
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                     'This offer is disabled: You already have this mount.')
            return
        end
    ...
    elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_NAMECHANGE) then
        ...
        if (not tile or not tile:hasFlag(TILESTATE_PROTECTIONZONE)) then
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_NETWORK,
                                     'You can change name only in Protection Zone.')
            return
        end

        local newName = extraInfo:lower():gsub('(%l)(%w*)', function(a, b)
        ...
        if (newName:len() < 3 or newName:len() > NETWORKMESSAGE_PLAYERNAME_MAXLENGTH) then
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                     'Your character name must be between 3 - ' .. NETWORKMESSAGE_PLAYERNAME_MAXLENGTH ..
                                         ' characters long.')
            return
        end
        ...
        if (matchAZ) then
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                     'Your name may only contain a-z, A-Z and spaces.')
            return
        end
        ...
        if (count > 1) then
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                     'Your name can not contain more than 2 words.')
            return
        end

        if (MonsterType(newName)) then
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                     'Your username contains a restricted word.')
            return
        elseif (Npc(newName)) then
            ...
            local words = {'owner', 'gamemaster', 'hoster', 'admin', 'staff', 'tibia', 'account', 'god', 'anal', 'ass',
                           'fuck', 'sex', 'hitler', 'pussy', 'dick', 'rape', 'adm', 'cm', 'gm', 'tutor', 'counsellor'}
    ...
    elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_PROMOTION) then
        GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                 'This offer is disabled: This category ain\'t done yet.')
        return
    ...
    elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_TEMPLE) then
        if (player:hasCondition(CONDITION_INFIGHT) or player:isPzLocked()) then
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                     'This offer is disabled: You can\'t use temple teleport in fight!')
            return
        end
    ...
    elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_POUCH) then
        ...
        if (pouch) then
            GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                     'This offer is disabled: You already have Gold Pouch.')
            return
        end
        ...
        if (inbox) then -- check if we have store inbox
            ...
            if (not player:addItem(26377, 1, false)) then
                GameStore.sendStoreError(player, GameStore.StoreErrors.STORE_ERROR_PURCHASE,
                                         'Please make sure you have free slot in your inventory.')
                return
            end
        end
    elseif (offer.type == GameStore.OfferTypes.OFFER_TYPE_ALLBLESSINGS) then
    ...
    db.asyncQuery('UPDATE `accounts` SET `coins` = ' .. balance .. ' WHERE `id` = ' .. player:getAccountId())
    GameStore.insertHistory(player:getAccountId(), GameStore.HistoryTypes.HISTORY_TYPE_NONE, offer.name,
                            -1 * offer.price, GameStore.CoinTypes.TIBIA_COIN)
...
function parseBuyStore(player, msg)
    ...
    db.asyncStoreQuery('SELECT `coins` FROM `accounts` WHERE `id` = ' .. player:getAccountId(), playerBuyStore,
                       player:getId(), offerId, productType, extraInfo)
end
...
function parseOpenTransactions(player, msg)
    ...
    local omsg = NetworkMessage()
    db.asyncStoreQuery('SELECT *, (select count(*) FROM `store_history` WHERE `account_id` = ' .. accountId ..
                           ') as `total` FROM `store_history` WHERE `account_id` = ' .. accountId ..
                           ' ORDER BY `time` DESC LIMIT ' .. offset .. ', ' .. entries .. ';', playerTransactionResult,
                       player:getId(), page, entries)
end
