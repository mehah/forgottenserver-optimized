function onStartup()
    ...
    db.asyncQuery('DELETE FROM `market_history` WHERE `inserted` <= ' ..
                      (os.time() - configManager.getNumber(configKeys.MARKET_OFFER_DURATION)))
        ...
        repeat
            ...
            db.asyncQuery(
                'INSERT INTO `account_ban_history` (`account_id`, `reason`, `banned_at`, `expired_at`, `banned_by`) VALUES (' ..
                    accountId .. ', ' .. db.escapeString(result.getDataString(resultId, 'reason')) .. ', ' ..
                    result.getDataLong(resultId, 'banned_at') .. ', ' .. result.getDataLong(resultId, 'expires_at') ..
                    ', ' .. result.getDataInt(resultId, 'banned_by') .. ')')
        ...
        repeat
            ...
            if house then
                ...
                if balance >= lastBid then
                    db.query('UPDATE `players` SET `balance` = ' .. (balance - lastBid) .. ' WHERE `id` = ' ..
                                 highestBidder)
    ...
    for i, town in ipairs(Game.getTowns()) do
        ...
        db.query('INSERT INTO `towns` (`id`, `name`, `posx`, `posy`, `posz`) VALUES (' .. town:getId() .. ', ' ..
                     db.escapeString(town:getName()) .. ', ' .. position.x .. ', ' .. position.y .. ', ' .. position.z ..
                     ')')
    end
end
