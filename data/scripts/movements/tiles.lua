...
function tile.onStepIn(creature, item, position, fromPosition)
    ...
    if Tile(position):hasFlag(TILESTATE_PROTECTIONZONE) then
        ...
        if depotItem then
            local depotItems =
                creature:getDepotLocker(getDepotId(depotItem:getUniqueId()), true):getItemHoldingCount() - itemAdjusting
            creature:sendTextMessage(MESSAGE_STATUS_DEFAULT, 'Your depot contains ' .. depotItems .. ' item' ..
...
function tile.onStepOut(creature, item, position, fromPosition)
    ...
    if GAME_FEATURE_STASH and creature:isPlayer() and
        (creature:isSupplyStashAvailable() or creature:isMarketAvailable()) then
