...
local items = {
    equipment = {
        ...
        [10022] = { -- worn firewalker boots
            [COMBAT_FIREDAMAGE] = {
                id = 9933,
                say = {
                    text = 'Take the boots off first.'
                }
            },
            slot = {
                type = CONST_SLOT_FEET,
                check = true
            }
        },
        [24716] = { -- werewolf amulet
            [COMBAT_NONE] = {
                ...
                effects = {
                    ...
                    success = CONST_ME_THUNDER
                },
                message = {
                    text = 'The amulet cannot be enchanted while worn.'
                }
            },
            slot = {
                type = CONST_SLOT_NECKLACE,
                check = true
            }
        },
        [24718] = { -- werewolf helmet
            [COMBAT_NONE] = {
                ...
                message = {
                    text = 'The helmet cannot be enchanted while worn.'
                },
                usesStorage = true
            },
            slot = {
                type = CONST_SLOT_HEAD,
                check = true
            }
        },
        charges = 1000,
    ...
    valuables = {
        [2146] = {
            id = 7759,
            shrine = {7508, 7509, 7510, 7511}
        }, -- small sapphire
        [2147] = {
            id = 7760,
            shrine = {7504, 7505, 7506, 7507}
        }, -- small ruby
        [2149] = {
            id = 7761,
            shrine = {7516, 7517, 7518, 7519}
        }, -- small emerald
        [2150] = {
            id = 7762,
            shrine = {7512, 7513, 7514, 7515}
        }, -- small amethyst
        soul = 2,
        mana = 300,
...
function enchanting.onUse(player, item, fromPosition, target, toPosition, isHotkey)
    ...
    if targetType.shrine then
        ...
        local isInSlot =
            targetType.slot and targetType.slot.check and target:getType():usesSlot(targetType.slot.type) and
                Player(target:getParent())
        if isInSlot then
            ...
            if targetItem.targetId then
                ...
                if targetItem.usesStorage then
                    ...
                    local storage = storages[itemId] and storages[itemId][targetId] and
                                        storages[itemId][targetId][vocationId]
                    if not storage then
                        return false
                    end
        ...
        player:getPosition():sendMagicEffect(targetItem.effects and
                                                 (isInSlot and targetItem.effects.failure or targetItem.effects.success) or
                                                 items.equipment.effect)
    end
    return true
end
