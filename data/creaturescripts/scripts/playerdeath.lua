...
function onDeath(player, corpse, killer, mostDamageKiller, unjustified, mostDamageUnjustified)
    ...
    db.asyncQuery(
        'INSERT INTO `player_deaths` (`player_id`, `time`, `level`, `killed_by`, `is_player`, `mostdamage_by`, `mostdamage_is_player`, `unjustified`, `mostdamage_unjustified`) VALUES (' ..
            player:getGuid() .. ', ' .. os.time() .. ', ' .. player:getLevel() .. ', ' .. db.escapeString(killerName) ..
    ...
    if byPlayer == 1 then
        ...
        if targetGuild ~= 0 then
            ...
            if killerGuild ~= 0 and targetGuild ~= killerGuild and isInWar(playerId, killer:getId()) then
                db.asyncQuery(
                    'INSERT INTO `guildwar_kills` (`killer`, `target`, `killerguild`, `targetguild`, `time`, `warid`) SELECT ' ..
                        db.escapeString(killerName) .. ', ' .. db.escapeString(player:getName()) .. ', ' .. killerGuild ..
                        ', ' .. targetGuild .. ', ' .. os.time() ..
                        ', `id` FROM `guild_wars` WHERE `status` = 1 AND ((`guild1` = ' .. killerGuild ..
                        ' AND `guild2` = ' .. targetGuild .. ') OR (`guild1` = ' .. targetGuild .. ' AND `guild2` = ' ..
                        killerGuild .. '))')
            end
        end
    end
