...
function getCreatureMaxHealth(cid, ignoreModifiers)
    local c = Creature(cid)
    return c and (ignoreModifiers and c:getBaseMaxHealth() or c:getMaxHealth()) or false
end
...
function getCreatureByName(name)
    local c = Creature(name)
    return c and c:getId() or false
end
...
function getCreatureSpeakType()
    debugPrint('Deprecated function(getCreatureSpeakType).')
    return true
end
function getCreatureLastPosition()
    debugPrint('Deprecated function(getCreatureLastPosition).')
    return true
end
function getCreatureStorageList()
    debugPrint('Deprecated function(getCreatureStorageList).')
    return true
end
function getCreatureGuildEmblem()
    debugPrint('Deprecated function(getCreatureGuildEmblem).')
    return true
end
function getCreaturePartyShield()
    debugPrint('Deprecated function(getCreaturePartyShield).')
    return true
end
...
function doRemoveCondition(cid, conditionType, ...)
    local c = Creature(cid)
    return c and
               (c:removeCondition(conditionType, CONDITIONID_COMBAT, ...) or
                   c:removeCondition(conditionType, CONDITIONID_DEFAULT, ...) or true)
end
function doRemoveConditions()
    debugPrint('Deprecated function(doRemoveConditions).')
    return true
end
function doCreatureSetSpeakType()
    debugPrint('Deprecated function(doCreatureSetSpeakType).')
    return true
end
function doCreatureSetGuildEmblem()
    debugPrint('Deprecated function(doCreatureSetGuildEmblem).')
    return true
end
function doCreatureSetPartyShield()
    debugPrint('Deprecated function(doCreatureSetPartyShield).')
    return true
end

function getCreatureCondition(cid, type, ...)
    local c = Creature(cid)
    return c and c:hasCondition(type, ...) or false
end
...
function getPlayerSpecialDescription()
    debugPrint('Deprecated function(getPlayerSpecialDescription).')
    return true
end
...
function getPlayerByGUID(guid)
    local p = Player(guid)
    return p and p:getId() or nil
end
function getPlayerByNameWildcard(name, ret)
    local p = Player(name)
    return ret and (p and RETURNVALUE_NOERROR or RETURNVALUE_PLAYERWITHTHISNAMEISNOTONLINE) or (p and p:getId() or nil)
end
function getPlayerSkullEnd(cid)
    local p = Player(cid)
    return p and p:getSkullTime() + os.time() or false
end
...
function getPlayerMagLevel(cid, ignoreModifiers)
    local p = Player(cid)
    return p and (ignoreModifiers and p:getBaseMagicLevel() or p:getMagicLevel()) or false
end
...
function getPlayerMaxMana(cid, ignoreModifiers)
    local p = Player(cid)
    return p and (ignoreModifiers and p:getBaseMaxMana() or p:getMaxMana()) or false
end
...
function getPlayerFightMode(cid)
    local p = Player(cid)
    return p and p:getFightMode() or false
end
function getPlayerSecureMode(cid)
    local p = Player(cid)
    return p and p:hasSecureMode() or false
end
...
function doPlayerSetNameDescription()
    debugPrint('Deprecated function(doPlayerSetNameDescription).')
    return true
end
function doPlayerSendChannelMessage(cid, author, message, type, channel)
    local p = Player(cid)
    return p and p:sendChannelMessage(author, message, type, channel) or false
end
...
function doPlayerSetSpecialDescription()
    debugPrint('Deprecated function(doPlayerSetSpecialDescription).')
    return true
end
...
function doPlayerAddStamina(cid, minutes)
    local p = Player(cid)
    return p and p:setStamina(p:getStamina() + minutes) or false
end
...
function doPlayerSetSkullEnd(cid, time, type)
    local p = Player(cid)
    return p and (p:setSkull(type) and p:setSkullEnd(time - os.time())) or false
end
...
if (CLIENT_VERSION < 900) then
    function doSendAnimatedText(pos, text, color, player)
        Game.sendAnimatedText(text, pos, color, player)
        return true
    end
else
    function doSendAnimatedText()
        debugPrint('Deprecated function(doSendAnimatedText).')
        return true
    end
end
...
function setPlayerMaxMana(cid, mana)
    local p = Player(cid)
...
function doCreateNpc(name, pos, ...)
    ...
    return n and n:setMasterPos(pos) or false
end
function doCreateMonster(name, pos, ...)
...
function getItemAttribute(uid, key)
    local i = Item(uid)
    return i and i:getAttribute(key) or false
end
function doItemSetAttribute(uid, key, value)
    local i = Item(uid)
    return i and i:setAttribute(key, value) or false
end
function doItemEraseAttribute(uid, key)
    local i = Item(uid)
    return i and i:removeAttribute(key) or false
end
...
function getSpectators(centerPos, rangex, rangey, multifloor, onlyPlayers)
    local result = Game.getSpectators(centerPos, multifloor or false, onlyPlayers or false, rangex, rangex, rangey,
                                      rangey)
    if #result == 0 then
        return nil
    end
...
function doPlayerTakeItem(cid, itemid, count)
    local p = Player(cid)
    return p and p:removeItem(itemid, count)
end
...
function getAccountByName(name)
    ...
    local resultId = db.storeQuery(
                         'SELECT `name` FROM `accounts` WHERE `id` = (SELECT `account_id` FROM `players` WHERE `name` = ' ..
                             db.escapeString(name) .. ' LIMIT 1) LIMIT 1')
    if resultId ~= false then
        local name = result.getString(resultId, 'name')
        result.free(resultId)
        return name
